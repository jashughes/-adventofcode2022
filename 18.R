...
get_neighbours <- function(r, d) d + rep(r, each = 6)
hash <- function(m) m %*% (100**(0:2))
hash_match <- function(m1, m2) match(hash(m1), hash(m2))
...
overlapping_faces <- function(inp) {
  ...
  for (i in seq_len(nrow(inp))) {
    for (j in i:nrow(inp)) {
      ol <- ol + is_adj(inp[i,], inp[j,])
    }
  }
  nrow(inp) * 6 - 2 * ol
}

bubble_expand <- function(p, seen, d, max_size = 3000) {
  explore <- matrix(p, nrow = 1)
  seen <- hash(p)
  while (nrow(explore) > 0) {
    nbs <- matrix(ncol = 3, nrow = 0)
    for (r in 1:(nrow(explore))) {
      nbs <- rbind(nbs, get_neighbours(explore[r,], d))
    }
    nbs <- nbs[!duplicated(nbs),]
    nbs <- pts[hash_match(nbs, pts[,1:3]),]
    ...
    if (length(seen) > max_size) return(FALSE)
  }
