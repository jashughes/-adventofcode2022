...
top2 <- function(vec) prod(sort(vec, decreasing = TRUE)[1:2])
pass_test <- function(n, div, if_T, if_F) ifelse(n %% div == 0, if_T, if_F)

play <- function(inv, op, test, ifT, ifF, f, reps = 20){
...
message("Part 1:", top2(play(inv,op,test,ifT,ifF, function(x) x %/% 3, 20)))
message("Part 2:", top2(play(inv,op,test,ifT,ifF, function(x) x%%test, 10000)))
